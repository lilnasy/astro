...
describe('Setting inlineStylesheets to "auto" in "static" output', () => {
	...
	it('Renders some <style> and some <link> tags', async () => {
		...
		expect($('link[rel=stylesheet]')).to.have.lengthOf(4);
	});
...
describe('Setting inlineStylesheets to "always" in "static" output with "assetsInlineLimit" set to 0', () => {
	let fixture;
...
async function stylesFromServer(app) {
	...
	const links = $('link[rel=stylesheet]');
	const hrefs = links.map((_, linkEl) => linkEl.attribs.href).toArray();
	const allLinkedStylesheets = await Promise.all(hrefs.map(async href => {
